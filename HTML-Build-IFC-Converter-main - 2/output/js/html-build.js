...
function main() {
	...
	console.log($(this)[0].innerHTML);
		$( this ).on("click", function(){
			...
			roomList = $(this).attr('roomnames'); // room = "['exmpl','string']"
			roomList = roomList.replace(/'/g, '"');	//JSON parse recognizes "" for strings
			roomList = JSON.parse(roomList);

			text = '<b>' + $(this).attr('name') + '</b><br>'
			+ 'Floor level: ' + $(this).attr('level')+'<br>'
			+ 'Elevation: ' + $(this).attr('elev') +'<br>'
...
jQuery('#plan').css("overflow-y", "scroll"); // adds scrollbar
}
